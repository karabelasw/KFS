-- ===================================================================
-- KFS_TEST - FULL SCHEMA CREATE SCRIPT
-- DESTRUCTIVE: drops and rebuilds the KFS_TES database from scratch.
-- Use for fresh dev environments only. NEVER run against a database
-- with data you need to keep — use -schema-migrations.sql instead
-- to bring an existing database forward without data loss.
-- ===================================================================
DROP DATABASE IF EXISTS KFS_TEST;
CREATE DATABASE KFS_TEST;
USE KFS_TEST;
...
VALUES ('system', 'system@kfs_test.local', (SELECT id FROM system_role WHERE code = 'SYSTEM'));
